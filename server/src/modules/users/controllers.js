...
export default {
  ...
  getUser: async (req, res) => {
    ...
    try {
      ...
      res.status(httpStatus.NOT_FOUND)
        .send(response.error(httpStatus.NOT_FOUND, 'User not found'));
    }
  },
  ...
  updateUser: async (req, res) => {
    ...
    if (update.nModified) {
      ...
      res.status(httpStatus.INTERNAL_SERVER_ERROR)
        .send(response.error(httpStatus.INTERNAL_SERVER_ERROR, 'Update user fail'));
    }
  },
  deleteUser: async (req, res) => {
    ...
    try {
      ...
      if (result) {
        ...
        res.status(httpStatus.INTERNAL_SERVER_ERROR)
          .send(response.error(httpStatus.INTERNAL_SERVER_ERROR, 'Delete user fail'));
      }
    } catch (err) {
      res.status(httpStatus.INTERNAL_SERVER_ERROR)
        .send(response.error(httpStatus.INTERNAL_SERVER_ERROR, err.message));
