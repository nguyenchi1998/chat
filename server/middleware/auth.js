...
export default {
  checkAuth: (req, res, next) => {
    try {
      ...
      req.user = jwt.verify(token, process.env.accessToken);
      next();
    } catch (ex) {
      return res
        .status(httpStatus.UNAUTHORIZED)
        .send(response.fail(httpStatus.UNAUTHORIZED, ex.message));
    }
  },
  checkRole: (...allowed) => {
    ...
    return async (req, res, next) => {
      try {
        ...
        return res
          .status(httpStatus.FORBIDDEN)
          .send(response.fail(httpStatus.FORBIDDEN, 'Access denied 1'));
      }
    };
  }
};
